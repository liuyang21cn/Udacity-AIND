...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    # initialize corner_state as all zeros
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        nextState = (nextx, nexty)
        cost = 1
        # copy corner_state and change to 1 if nextstate at corner
...
def cornersHeuristic(state, problem):
  ...
  "*** YOUR CODE HERE ***"
  # get position and corner state
  x, y = state[0]
  corner_state = list(state[1])
  # initialize ans
  ans = float('inf')
  # calculate distance when 0 in corner_state
  while 0 in corner_state:
      for idx, corner in enumerate(corners):
          if corner_state[idx] == 0:
              # manhattanDistance
              temp = abs(x-corner[0])+abs(y-corner[1])
              # calculate obstacles
              x0, x1 = min(x, corner[0]), max(x, corner[0])
              y0, y1 = min(y, corner[1]), max(y, corner[1])
...
def foodHeuristic(state, problem):
  ...
  for food in foodGrid.asList():
      s += util.manhattanDistance(food, position)
  return s
  # food_count = foodGrid.count()
  # return food_count/s if s != 0 else 0
