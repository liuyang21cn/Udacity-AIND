...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # create empyt set for current level
        a_level = set()
        # loop over actions and create node
        for action in self.all_actions:
            Node_a = PgNode_a(action)
            # find proposed PgNode_a has prenodes that are a subset of the previous S level
            if Node_a.prenodes.issubset(self.s_levels[level]):
                # add action node to current level and link parents and children
                a_level.add(Node_a)
        ...
        # append current level to a_levels
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # find effects for node_a1 and node_a2
        ...
        # if intersection between a1's effect_add and a2's effect_rem is not 0,
        # or a2's effect_add and a1's effect_rem is not 0, return True
        # AIMA p387
        if len(a1_effect_add.intersection(a2_effect_rem)) != 0 or \
            len(a1_effect_rem.intersection(a2_effect_add)) != 0:
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # loop over one node's effect_add, check if it's in another node's precond_neg
        # AIMA p387
        ...
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True
        # loop over one node's effect_rem, check if it's in another node's precond_pos
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # loop over parent of a1 and 2, use is_mutex to check
        # AIMA p387
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # For negation_mutex, we need both nodes have same symbol and different pos
        # node_s1.show()
        # node_s2.show()
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # loop over previous level of actions/parents, check is_mutex
        # return false if preconditions of s1 and s2 are not mutex
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        # TODO implement
        # for each goal in the problem, loop over each level and try to find a
        # match, if match length not zero, then add current level index to sum
        # and go to find match for next goal
        # AIMA p388
        for goal in self.problem.goal:
            for level in range(len(self.s_levels)):
                if len([node for node in self.s_levels[level] if node.symbol == goal and node.is_pos]) != 0:
                    level_sum += level
                    break
        return level_sum
